...
//////////////////////////
// SPA webserver
gulp.task('webserver', ['scss', 'templates', 'concatJS', 'buildJS'], function() {
    gulp.src('../')
        .pipe(
...
gulp.task('templates', function() {
    ...
    gulp.src('./templates/**/*.html')
        ...
        .pipe(insert.prepend(config.namespace.toUpperCase() + '.templates = '))
        .pipe(insert.append(';'))
...
gulp.task('buildJS', function() {
    ...
    gulp.src(src)
        .pipe(sourcemaps.init())
        .pipe(babel({ presets: ['es2015'] }).on('error', function(err){ console.log(err.message) }))
        .pipe(concat( config.namespace + '.js'))

        .pipe(replace(/\/mocks\//g, config.server.path + '/mocks/'))
        .pipe(replace(/\/protection\\/g, config.server.path + '\\/protection\\'))
        .pipe(replace(/\/protection\//g, config.server.path + '/protection\/'))
