 (function($){
    ...
    $(document).on('click', '.resizeToggler', function(){
        var targetName = this.getAttribute('for');
        ...
        // toggle targte class
        $('.' + targetName).toggleClass('minimize');
    })
...
// "Support" Testers
    ...
    $.fn.svgAddClass = function(className){
        return this.each(function(){
            ...
            if( $elm.attr('class') && $elm.attr('class').split(' ').indexOf(className) != -1 )
                return;

            $elm.attr('class', value + ' ' + className);
        });
    }
    ...
    $.fn.svgRemoveClass = function(className){
        return this.each(function(){
            ...
            $elm.attr('class', value.replace(className,'') );
        });
    }
...
// contentEditable placeholders
    (function(){
        $(document)
            ...
            function onFocus(){
                if( !EMC.utilities.string.normalizeContentEditable(this.innerHTML).trim() ){
                    this.innerHTML = '';
